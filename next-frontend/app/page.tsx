...
export default function Home() {
  ...
  const login = async () => {
    window.location.assign('https://auth.orme.dev/realms/movies/protocol/openid-connect/auth?response_type=code&scope=openid%20write%20read&client_id=movies&redirect_uri=https://movies.orme.dev/');
  }
  ...
  const fetchData = async () => {
    if(!token) return
    const data = await fetch(`${window.location.origin}/api/movies/all`, {headers:
  ...
  return (
    <main>
      ...
      {token && (
        <form>
          <input type={"number"} name={"movieId"} value={movieId} onChange={(e) => setMovieId(e.target.value)}/>
